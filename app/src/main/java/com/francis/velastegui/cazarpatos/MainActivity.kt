...
class MainActivity : AppCompatActivity() {
    ...
    private fun eliminarPuntajeDelUsuarioActual() {
        val nombreJugador = textViewUser.text.toString()
        val db = Firebase.firestore
        db.collection("ranking")
            .whereEqualTo("username", nombreJugador)
            .get()
            .addOnSuccessListener { documents ->
                if (!documents.isEmpty) {
                    val idDocumento = documents.documents[0].id
                    eliminarPuntajeJugador(idDocumento)
                } else {
                    Toast.makeText(this, "No se encontró el puntaje del usuario", Toast.LENGTH_LONG).show()
                }
            }
            .addOnFailureListener { exception ->
                Log.w(EXTRA_LOGIN, "Error al buscar el puntaje para eliminar", exception)
                Toast.makeText(this, "Error al buscar el puntaje", Toast.LENGTH_LONG).show()
            }
    }
